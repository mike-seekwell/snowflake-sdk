...
StateConnecting.prototype.continue = function(options)
{
  ...
  {
    method   : 'POST',
    url      : buildLoginUrl(this.connectionConfig),
    json     : json,
    scope    : this,
    callback : function(err, body)
    {
      // clear credential-related information
      this.connectionConfig.clearCredentials();
      ...
      {
        // TODO: wrap the error into a connectionerror here

        // TODO: if it's a network error, should we go back to the preconnected
        // state?
...
StateConnected.prototype.request = function(options)
{
  ...
  options.callback = function(err, body)
  {
    ...
    else
    {
      ...
      else if ((err.code === '390104') || (err.code === '390111'))
      {
        // enqueue the request operation
        this.snowflakeService.enqueueRequest(options);
...
StateRenewing.prototype.enter = function(context)
{
  ...
  this.createMasterTokenRequest(
  {
    ...
    callback: function(err, body)
    {
      ...
      {
        // if the master token has expired, transition to the disconnected state
        if (err.code === '390114')
        {
          this.snowflakeService.transitionToDisconnected();
        }
        ...
        else // if the renewal failed for some other reason, we're disconnected
        {
