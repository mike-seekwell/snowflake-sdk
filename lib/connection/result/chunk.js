/*
 * Copyright (c) 2015-2016 Snowflake Computing Inc. All rights reserved.
 */
...
function Chunk(options)
{
  ...
  // if the result is small (i.e. not persisted on S3), there's no compressed
  // and uncompressed size, so default to -1
...
/**
 * Returns the compressed size of this chunk's contents on S3. A value of -1
 * means the contents of this chunk are not stored on S3.
 *
 * @returns {Number}
 */
...
/**
 * Returns the uncompressed size of this chunk's contents on S3. A value of -1
 * means the contents of this chunk are not stored on S3.
 *
 * @returns {Number}
 */
...
{
  var chunkStart = this._startIndex, chunkEnd = this._endIndex;
...
Chunk.prototype.load = function(callback)
{
  ...
  {
    // issue a request to load the chunk's contents from S3
    this._services.s3.getObject(
    {
      url      : this._url,
