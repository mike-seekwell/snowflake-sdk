    ...
    statementOptions, services, connectionConfig)
{
  ...
  {
    statementContext.requestId = uuid.v1();
  }
...
function fetchRowsFromResult(options, statement, context)
{
  ...
  operation.on('complete', function(err, continueCallback)
  {
    ...
    if (Errors.isS3Error(err) && err.response &&
...
{
  var bindsMap = {};
  for (var index = 0, length = bindsArray.length; index < length; index++)
  {
    var value = bindsArray[index];
...
function sendSfRequest(statementContext, options, appendQueryParamOnRetry)
{
  ...
  options.callback = function(err)
  {
    ...
    if ((numRetries < maxNumRetries) && err && err.response &&
        ...
        (err.response.statusCode === 408)))
    {
      ...
      // double the sleep time until we've reached the maximum allowed value
      sleep = Math.min(connectionConfig.getRetrySfMaxSleepTime(), sleep * 2);
